...
def make_random_particle():
    e = numpy.random.rand(ARRAY_LENGTH)
    x = numpy.random.rand(ARRAY_LENGTH)
    y = numpy.random.rand(ARRAY_LENGTH)
    z = numpy.random.rand(ARRAY_LENGTH)
    vector = numpy.column_stack((e, x, y, z))
    vector = vector.ravel().view(
        [
            ('e', 'f8'), ('x', 'f8'),
            ('y', 'f8'), ('z', 'f8')
        ]
...
@pytest.fixture()
def four_vector():
    return vectors.FourVector(make_random_particle())
...
def test_four_vector_get_array_returns_array(four_vector):
    assert isinstance(four_vector.get_array(), numpy.ndarray)
...
def test_four_vector_length(four_vector):
    assert isinstance(four_vector.length, numpy.ndarray)


def test_three_vector_theta(three_vector):
    assert isinstance(three_vector.theta, numpy.ndarray)


def test_four_vector_phi(four_vector):
    assert isinstance(four_vector.phi, numpy.ndarray)


def test_three_vector_sin_theta(three_vector):
    assert isinstance(three_vector.sin_theta, numpy.ndarray)


def test_four_vector_cos_theta(four_vector):
    assert isinstance(four_vector.cos_theta, numpy.ndarray)
...
def test_three_vector_length_squared(three_vector):
    assert isinstance(three_vector.length_squared, numpy.ndarray)


def test_four_vector_length_squared(four_vector):
    assert isinstance(four_vector.length_squared, numpy.ndarray)
